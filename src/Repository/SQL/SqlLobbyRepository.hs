...
instance LobbyRepository SqlLobbyRepositoryBackend where
  ...
  getInviterForUser (SqlLobbyRepositoryBackend pool) = getInviterForUserSQL pool
...
getInviterForUserSQL :: Pool SqlBackend -> T.Text -> T.Text -> IO (Maybe T.Text)
getInviterForUserSQL pool gameLobbyId userId =
  withPool pool $ do
    results <- rawSql
      "SELECT u.username FROM lobby_invite li JOIN \"user\" u ON u.ident = li.invite_from WHERE li.lobby = ? AND li.invite_to = ? LIMIT 1"
      [PersistText gameLobbyId, PersistText userId]
    case results of
      (Single (Just username) : _) -> return (Just username)
      _ -> return Nothing
